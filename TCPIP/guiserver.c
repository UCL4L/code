...
void send_only(char name[50],char loadsend[BUFFER_SIZE]){
	int j;
	for(j=0;j<THREAD_NUMBER;j++){
		if((users[j].login==1)&&(strcmp(users[j].name,name)==0)){
			send(users[j].client_fd, loadsend, strlen(loadsend), 0);		
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	for(i=0;i<THREAD_NUMBER;i++)
		users[i].login=0;
	i=0;
	if (listen(sockfd, THREAD_NUMBER) == -1)
	{	
...
编译：
gcc guiserver.c -o server -lpthread
gcc guiclient.c -o client -luuid -lpthread `pkg-config --cflags --libs gtk+-2.0`
运行：
./server
./client